			...
			for (c1 = 0; c1 < w->d[0]; ++c1)	 /* output channel */                                                         \
				for (c0 = 0; c0 < w->d[1]; ++c0) /* input channel */                                                          \
					...
					{ /* kernel row */                                                                                        \
						...
						{ /* output row */                                                                                    \
							/* array indexes */                                                                               \
							int _xx_idx0 = ((n * q->d[1] + c0) * q->d[2] + ii) * q->d[3];                                     \
							int _yy_idx0 = ((n * p->d[1] + c1) * p->d[2] + i) * p->d[3];                                      \
							int _xx_idx1 = ((n * q->d[1] + c0) * q->d[2] + ii) * q->d[3] + 1;                                 \
							int _yy_idx1 = ((n * p->d[1] + c1) * p->d[2] + i) * p->d[3] + 1;                                  \
							int _xx_idx2 = ((n * q->d[1] + c0) * q->d[2] + ii) * q->d[3] + 2;                                 \
							int _yy_idx2 = ((n * p->d[1] + c1) * p->d[2] + i) * p->d[3] + 2;                                  \
							int _xx_idx3 = ((n * q->d[1] + c0) * q->d[2] + ii) * q->d[3] + 3;                                 \
							int _yy_idx3 = ((n * p->d[1] + c1) * p->d[2] + i) * p->d[3] + 3;                                  \
							float *_xx0 = &(_x)[_xx_idx0];                                                                    \
							float *_yy0 = &(_y)[_yy_idx0];                                                                    \
							float *_xx1 = &(_x)[_xx_idx1];                                                                    \
							float *_yy1 = &(_y)[_yy_idx1];                                                                    \
							float *_xx2 = &(_x)[_xx_idx2];                                                                    \
							float *_yy2 = &(_y)[_yy_idx2];                                                                    \
							float *_xx3 = &(_x)[_xx_idx3];                                                                    \
							float *_yy3 = &(_y)[_yy_idx3];                                                                    \
							if (x_padded)                                                                                     \
							{                                                                                                 \
								memcpy(x_padded + aux[1].pad[0], _xx0, q->d[3] * sizeof(float));                              \
								_xx0 = x_padded + aux[1].pad[0];                                                              \
								memcpy(x_padded + aux[1].pad[0], _xx1, q->d[3] * sizeof(float));                              \
								_xx1 = x_padded + aux[1].pad[0];                                                              \
								memcpy(x_padded + aux[1].pad[0], _xx2, q->d[3] * sizeof(float));                              \
								_xx2 = x_padded + aux[1].pad[0];                                                              \
								memcpy(x_padded + aux[1].pad[0], _xx3, q->d[3] * sizeof(float));                              \
								_xx3 = x_padded + aux[1].pad[0];                                                              \
							}                                                                                                 \
							_row_func(_xx0, _ww, _yy0, w->d[3], p->d[3], aux[1].stride, aux[1].pad[0], (_tmp));               \
							_row_func(_xx1, _ww, _yy1, w->d[3], p->d[3], aux[1].stride, aux[1].pad[0], (_tmp));               \
							_row_func(_xx2, _ww, _yy2, w->d[3], p->d[3], aux[1].stride, aux[1].pad[0], (_tmp));               \
							_row_func(_xx3, _ww, _yy3, w->d[3], p->d[3], aux[1].stride, aux[1].pad[0], (_tmp));               \
						} /* ~i */                                                                                            \
					}	  /* ~k, c0, c1, n */                                                                                 \
	} while (0)
