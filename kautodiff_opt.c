...
static inline float kad_sdot(int n, const float *x, const float *y) /* BLAS sdot using SSE */
{
    ...
    for (i = 0; i < n8; i += 16)
    {
        __m256 vx1, vx2, vy1, vy2;
        vx1 = _mm256_loadu_ps(&x[i]);
        vx2 = _mm256_loadu_ps(&x[i + 8]);
        vy1 = _mm256_loadu_ps(&y[i]);
        vy2 = _mm256_loadu_ps(&y[i + 8]);
        vs1 = _mm256_add_ps(vs1, _mm256_mul_ps(vx1, vy1));
        vs2 = _mm256_add_ps(vs2, _mm256_mul_ps(vx2, vy2));
    }
    for (s = 0.; i < n; ++i)
        s += x[i] * y[i];
    _mm256_storeu_ps(t, vs1);
    s += t[0] + t[1] + t[2] + t[3] + t[4] + t[5] + t[6] + t[7];
    _mm256_storeu_ps(t, vs2);
    s += t[0] + t[1] + t[2] + t[3] + t[4] + t[5] + t[6] + t[7];
    return s;
}
